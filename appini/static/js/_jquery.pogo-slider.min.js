/* PgSlider */
(function(d,c,b,e){
    function a(f,h,g){ if(h.charAt(0)==="*") {f[h.substring(1)]=g} 
                       else { f["-ms-"+h]=g; 
                              f["-webkit-"+h]=g;
                              f[h]=g} }
                d.fn.precss=function(g){
                                        ...
                                        for(style in g) { if(g.hasOwnProperty(style)){a(f,style,g[style])}}}
                                        else{ a( f,arguments[0],arguments[1])}
                                              this.css(f);
                                              return this}} )
...
(function(d,f,g,a){
  var c=function(n){
    ...
    if(m===false){
       ...
       for(var k=0; k<l.length;k++) {if(o.style[l[k]+j]!==a){m=true;break} } }
       return m };
...
_init:function(){
    var k=this;
        k.$element.find(".pogoSlider-slide").each( function(){ var n=[];
    d(this).data("original-styles",d(this).attr("style"));
    d(this).find(".pogoSlider-slide-element").each(
    ...
    if(j>0){
       ...
       k.$element.find("img").one("load",function(){if(++l===j){d(".pogoSlider-loading").remove();
       k._onSliderReady()}}).each(function(){if(this.complete){d(this).trigger("load")}})}
       else{k._onSliderReady()}},
            
       _onSliderReady:function(){
            var j=this;
            if(j.settings.autoplay){j.slideStartTime=new Date();
            ...
            if(j.settings.preserveTargetSize){
               ...
               if(j.settings.responsive){d(f).on("resize",function(){j._preserveTargetSize()})}}
               ...
               if(j.settings.pauseOnHover){
                ...
                for(var k=0;k<j.slides.length;k++){
                    d('<li data-num="'+k+'"><button class="pogoSlider-nav-btn"></button></li>').appendTo(l).on("click",function(){j.toSlide(d(this).data("num"))})}},
                    getAppliedProps:function(o){var y=g.styleSheets;
                    var m=new RegExp("{(.+)}");
                ...
                for(var q=0;q<p.length;q++){
                    var x=p[q].split(":")[0];
                    if(x&&w.indexOf(x)===-1){w.push(x)}}

                for(var v in y){if(y.hasOwnProperty(v)){
                    var u=y[v].rules||y[v].cssRules;
                    for(var l in u){ if(o.matches(u[l].selectorText)){
                        var t=m.exec(u[l].cssText.replace(/ /g,""));
                        if(t){var z=t[1].split(";");
                        for(var s=0;s<z.length;s++){var n=z[s].split(":")[0];
                            if(n&&w.indexOf(n)===-1){w.push(n)}}
                    }}}}}
                return w },
...
if(!d.data(k,"originalStyles")){d.data(k,"originalStyles",d(this).attr("style"))}for(var s=0;s<r.length;s++){var o=p.getPropertyValue(r[s]);
    if(n.test(o)&&l.test(o)){
       var t=l.exec(o);
       var q=n.exec(o);
    if(j.test(q[0])){u[r[s]]=Math.ceil(t[0]*m)+q[0]}else{u[r[s]]=(t[0]*m)+q[0]}}}d(this).css(u)})},
    
 _createProgessBar:function(){
    var k="";k+='<div class="pogoSlider-progressBar">';
    k+='<div class="pogoSlider-progressBar-duration"></div>';k+="</div>";
...
_slideTimeout:function(j){
    ...
    l=k.slideTimeoutId=setTimeout(function(){
        if(!k.paused&&l===k.slideTimeoutId){k._changeToNext()}},j)},
...
resume:function(){
    if(this.settings.autoplay){
    this.paused=false;this.slideStartTime=new Date();
    for(var j=0;j<this.slides[this.currentSlideIndex].children.length;j++){
...
_onSlideStart:function(k){
    ...
    if(this.settings.autoplay){
       ...
       if(this.settings.displayProgess&&!this.paused){
          this.slides[k].$element.find(".pogoSlider-progressBar-duration").css("width","0").animate({"width":"100%"},
...
_onSlideEnd:function(l){
    ...
    if(this.paused){k=this.slides[l].totalSlideDuration-this.slideTimeRemaining;
       for(var j=0;j<this.slides[l].children.length;j++){
           this.slides[l].children[j].$element.precss("animation-play-state","")}}else{k=this.slides[l].totalSlideDuration-(this.slideTimeRemaining-((new Date())-this.slideStartTime))}
...
_generateARandomArray:function(q){
    ...
    for(var o=l.length-1;o>0;o--){
        ...
        var m=l[o];l[o]=l[n];l[n]=m}return l},

slideTransitions:{fade:function(j,l){
                var k=this.slides[l];
...
slide:function(j,k){
    var l;
    if(k===0&&j===this.slides.length-1){l="slideLeft"}
    else{ if(j===0&&k===this.slides.length-1){l="slideRight"}
          else{if(k>j){l="slideLeft"}else{l="slideRight"}}}
          return this.slideTransitions[l].apply(this,[j,k])},
          
verticalSlide:function(j,k){
    var l;
    if(k===0&&j===this.slides.length-1){l="slideUp"}else{
        if(j===0&&k===this.slides.length-1){l="slideDown"}else{
            if(k>j){l="slideUp"}else{l="slideDown"}}}
    return this.slideTransitions[l].apply(this,[j,k])},
...
slideDown:function(k,m){
    ...
    return function(){j.slides[k].$element.removeClass("pogoSlider-animation-downOut");
...
expandReveal:function(k,m){
    ...
    l.$element.css({"opacity":1,"z-index":j.settings.baseZindex});
    return function(){j.$element.css("overflow","");
...
verticalSplitReveal:function(l,n){
    ...
    k._createSlideSlices(l,1,2);var j=k.slides[l].$element.find(".pogoSlider-slide-slice");
    j.precss("animation-duration",m.duration+"ms");j.eq(0).addClass("pogoSlider-animation-leftOut");
    j.eq(1).addClass("pogoSlider-animation-rightOut")},
    
horizontalSplitReveal:function(l,n){var k=this;
    var m=k.slides[n];k.slides[l].$element.css("z-index",k.settings.baseZindex+1);
    m.$element.css({"opacity":1,"z-index":k.settings.baseZindex});
...
zipReveal:function(l,n){
    ...
    var j=k.slides[l].$element.find(".pogoSlider-slide-slice");j.precss("animation-duration",m.duration+"ms");
    j.each(function(o){if(o%2===0){d(this).addClass("pogoSlider-animation-upOut")}
        else{d(this).addClass("pogoSlider-animation-downOut")}})},
...
barReveal:function(o,q,n){
    ...
    j.each(function(r){
        if(n==="down"){d(this).addClass("pogoSlider-animation-downOut").precss("animation-delay",l*r+"ms")} 
        else { d(this).addClass("pogoSlider-animation-upOut").precss("animation-delay",l*r+"ms")}})},

blocksReveal:function(k,j){
    var u=this;var q=u.slides[j];
    var t=0;if(u.settings.preserveTargetSize){t=parseFloat(u.$element.css("padding-bottom"))}
    else{t=u.$element.height()}
    ...
    var o=q.duration/(n.length+1);var s=o*2;n.precss("animation-duration",s+"ms");
    for(var m=0;m<n.length;m++){n.eq(p.pop()).precss("animation-delay",(o*m)+"ms").addClass("pogoSlider-animation-blocksReveal")}},

fold:function(j,k){
    var l;
    if(k===0&&j===this.slides.length-1){l="foldLeft"}
    else{ if(j===0&&k===this.slides.length-1){l="foldRight"}
        else{if(k>j){l="foldLeft"}
            else{l="foldRight"}}}
    return this.slideTransitions[l].apply(this,[j,k])},
...
foldLeft:function(l,k){
    ...
    var p=r.eq(1);var o=n.eq(0);
    var j=n.eq(1);q.$element.append(p.detach());
    ...
    return function(){s.$element.css("overflow","");
    s._slideCleanup(k,true)}}},
...
slide:function(j,k){
    var l;if(j>k&&j===this.slides.length-1&&k===0){l="slideLeft"}
    else{ if(j<k&&j===0&&k===this.slides.length-1){l="slideRight"}
        else{if(j<k){l="slideLeft"}else{l="slideRight"}}}    
    return this.slideTransitions[l].apply(this,[j,k])},
    
verticalSlide:function(j,k){
    var l;if(j>k&&j===this.slides.length-1&&k===0){l="slideUp"}
    else{if(j<k&&j===0&&k===this.slides.length-1){l="slideDown"}
        else{if(j<k){l="slideUp"}else{l="slideDown"}}}
    return this.slideTransitions[l].apply(this,[j,k])},
...
slideRevealLeft:function(j,l){
    var k=this.slides[l];this.slides[j].$element.css("z-index",this.settings.baseZindex+1).animate({left:"-100%"},k.duration);
...
verticalSplitReveal:function(l,n){
    ...
    var j=k.slides[l].$element.find(".pogoSlider-slide-slice");j.eq(0).animate({"left":"-50%"},m.duration);
...
horizontalSplitReveal:function(l,n){
    ...
    k.slides[l].$element.css("z-index",k.settings.baseZindex+1);
    m.$element.css({"opacity":1,"z-index":k.settings.baseZindex});
    k._createSlideSlices(l,2,1);var j=k.slides[l].$element.find(".pogoSlider-slide-slice");
...
zipReveal:function(n,p){
    ...
    l.each(function(q){if(q%2===0){d(this).delay(k*q).animate({"top":"100%"},j)}
    else{d(this).delay(k*q).animate({"top":"-100%"},j)}})},
...
barReveal:function(o,q,n){
    ...
    var j=k*2;
    l.each(function(r){if(n==="down"){d(this).delay(k*r).animate({"top":"100%"},j)}
    else{d(this).delay(k*r).animate({"top":"-100%"},j)}})}
    }};

    d.fn[e]=function(j){
        this.each(function(){
          if(!d.data(this,"plugin_"+e)){d.data(this,"plugin_"+e,new h(this,j))}});
